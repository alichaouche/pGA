...
void calculCoutCoupeEtFitnessSVTC(partitionSVTC* populationSVTC)
{
    ...
    {
        (populationSVTC+i)->coutCoupe = 0; /// réinitialisation de la valriable après chaque itération (individus de la population)
                                                                            ...
                                                                            int iteration , int *bestSolutionIteration , int *nbrApparition)
{
        ...
        crossOverSVTC(populationSVTC2,populationSVTC1); /// nbrNoeuds représente la taille des genotype
...
#if writingPopulationInFile
        writeBestSolutionInFileSVTC((populationSVTC1+bestSolution),outputFileSVTC,iteration);
#endif // writingPopulationInFile
...
void checkContrainstAndFitnessPenalizationSVTC(partitionSVTC *populationSVTC)
{
    ...
    for(i=0; i<taillePopulation; i++)
    {
        (populationSVTC+i)->nbrCluster=0; /// le nombre de cluster doit etre initialisé à 1 pour prendre en consédiration le premier cluster
